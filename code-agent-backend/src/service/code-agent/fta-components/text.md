...
Text是Taro中的文本显示组件，相当于React Native中的`<Text>`和Web中的`<span>`。Text组件支持嵌套使用，是展示文本内容的核心组件。
...
| decode | `boolean` | `false` | 否 | 是否解码HTML实体字符 |
...
| hoverStyle | `CSSProperties` | - | 否 | 按下去的样式（RN端特有） |
...
export default function NestedText() {
  return (
    <Text>
      这是一个<Text style={{ color: '#ff4d4f' }}>红色</Text>的
      <Text style={{ fontWeight: 'bold' }}>粗体</Text>文本。
    </Text>
  )
}
```
...
export default function RichText() {
  return (
    <View>
      <Text>
        这是一个<Text style={{ color: '#ff4d4f' }}>红色</Text>、
        <Text style={{ color: '#1890ff' }}>蓝色</Text>、
...
export default function FontCompatibility() {
  return (
    <View>
      ...
      <Text style={{ fontFamily: 'PingFang SC, sans-serif' }}>
        苹果系统字体
      </Text>
...
- [React Native Text文档](https://reactnative.dev/docs/text)
- [CSS字体属性](https://developer.mozilla.org/zh-CN/docs/Web/CSS/font-family)