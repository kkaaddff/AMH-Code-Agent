...
View是Taro中最基础的UI组件，相当于React Native中的`<View>`和Web中的`<div>`。View组件可以嵌套使用，是所有布局和UI的基础构建块。
...
| 属性名 | 类型 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- | --- |
| catchMove | `boolean` | `false` | 否 | 是否以catch的形式绑定touchmove事件 |
...
| hoverStyle | `CSSProperties` | - | 按下去的样式（RN端特有） |
...
| 属性 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| catchMove | `boolean` | `false` | 是否以catch的形式绑定touchmove事件 |
...
- **H5端**: 支持完整的CSS属性
- **RN端**: 只支持Flexbox布局，不支持传统的CSS属性
- **小程序端**: 样式有限制，推荐使用Flexbox
