...
export class GithubProvider {
  ...
  private saveState(): void {
    try {
      ...
      fs.writeFileSync(this.authFile, JSON.stringify(this.state, null, 2), 'utf-8');
    } catch (error) {
  ...
  async access(): Promise<string | undefined> {
    ...
    if (this.state.copilot_token && this.state.copilot_expires && this.state.copilot_expires > Date.now()) {
