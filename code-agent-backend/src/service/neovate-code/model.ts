...
export const defaultModelCreator = (name: string, provider: Provider): LanguageModelV2 => {
...
export const providers: ProvidersMap = {
  'github-copilot': {
    ...
    async createModel(name, provider, globalConfigDir) {
      ...
      if (!token) {
        throw new Error('Failed to get GitHub Copilot token, use /login to login first');
      }
      return createOpenAI({
  ...
  google: {
    ...
    models: {
      ...
      'gemini-2.5-flash-preview-09-2025': models['gemini-2.5-flash-preview-09-2025'],
  ...
  siliconflow: {
    ...
    models: {
      ...
      'Qwen/Qwen3-Coder-480B-A35B-Instruct': models['qwen3-coder-480b-a35b-instruct'],
...
function mergeConfigProviders(
  ...
  Object.entries(configProviders).forEach(([providerId, config]) => {
    let provider = mergedProviders[providerId] || ({} as Provider);
    provider = defu(config, provider);
...
export async function resolveModelWithContext(name: string | null, context: Context) {
  ...
  const model = modelName
    ? await resolveModel(modelName, finalProviders, hookedModelAlias, context.paths.globalConfigDir)
    : null;
  return {
...
export async function resolveModel(
  ...
  assert(provider, `Provider ${providerStr} not found, valid providers: ${Object.keys(providers).join(', ')}`);
  ...
  model.id = modelId;
  let m: LanguageModelV2 | Promise<LanguageModelV2> = provider.createModel(modelId, provider, globalConfigDir);
  if (isPromise(m)) {
    m = await m;
  }
  return {
