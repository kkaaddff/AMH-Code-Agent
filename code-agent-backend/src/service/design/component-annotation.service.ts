...
export class DesignComponentAnnotationService {
  ...
  public async saveAnnotation(
    ...
    const existing = payload.version ? await this.annotationModel.findOne({ designId, version: payload.version }) : null
    ...
    if (existing) {
      ...
      if (latestAnnotation && targetVersion <= (latestAnnotation.version ?? 0) && !payload.force) {
        throw new MidwayHttpError('Annotation version must be greater than current latest version', HttpStatus.CONFLICT)
      }
    } else {
  ...
  public async diffAnnotations(designId: string, params: DiffDesignAnnotationQuery): Promise<DiffChangeSetItem[]> {
    ...
    for (const [nodeId, newNode] of toMap.entries()) {
      if (!fromMap.has(nodeId)) {
        changes.push({ nodeId, changeType: 'added', detail: { after: newNode } })
      } else {
        const oldNode = fromMap.get(nodeId)
        if (JSON.stringify(oldNode) !== JSON.stringify(newNode)) {
          changes.push({ nodeId, changeType: 'updated', detail: { before: oldNode, after: newNode } })
        }
      }
    }
    ...
    for (const [nodeId, oldNode] of fromMap.entries()) {
      if (!toMap.has(nodeId)) {
        changes.push({ nodeId, changeType: 'removed', detail: { before: oldNode } })
      }
    }
