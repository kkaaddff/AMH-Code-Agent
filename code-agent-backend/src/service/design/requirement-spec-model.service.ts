...
export class RequirementSpecModelService {
    ...
    nodes.forEach((node) => {
      ...
      const metrics =
        node.width && node.height
          ? `尺寸：${Math.round(node.width)}×${Math.round(node.height)}`
          : undefined;
      const childInfo = node.childCount
        ? `子节点：${node.childCount}`
        : undefined;
      const info = [metrics, childInfo].filter(Boolean).join("，");
      lines.push(
        `${indent}- ${labelParts.join(" ")}${info ? `（${info}）` : ""}`
      );
    });
    return lines.join("\n");
  }

  private buildTemplateInstructions(templateKey?: string): {
    preface: string;
    constraints: string;
  } {
    ...
    const defaultConstraints1 = fs.readFileSync(
      path.join(__dirname, "ai-code-agent-spec-compact.md"),
      "utf-8"
    );
  ...
  private buildPrompt(
    ...
    const contextLines = [
      ...
      annotationSchemaVersion
        ? `标注 Schema：${annotationSchemaVersion}`
        : undefined,
  ...
  private preparePrompt(params: RequirementSpecGenerationParams): string {
    const annotationSummary = this.formatAnnotationSummary(
      this.flattenAnnotation(params.rootAnnotation)
    );
    ...
    try {
      ...
      console.warn(
        "[RequirementSpecModelService] 写入 prompt 调试文件失败",
        error
      );
    }
  ...
  private async requestModel(
    prompt: string
  ): Promise<{ content: string; usage?: Record<string, any> } | null> {
    try {
      ...
      if (result.success) {
        ...
        console.warn(
          "[RequirementSpecModelService] 模型调用失败，将回退至模板生成。",
          result.error
        );
        return null;
      }
  ...
  public async streamSpecification(
    ...
    try {
      ...
      console.warn(
        "[RequirementSpecModelService] 流式生成失败，将回退至非流式生成。",
        error
      );
    }
