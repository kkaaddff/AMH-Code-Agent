...
export class DocumentReferenceSwagger {
  ...
  @ApiProperty({ description: '同步状态', enum: ['pending', 'syncing', 'synced', 'failed', 'completed'] })
  status: string
...
export class PageSwagger {
  ...
  @ApiProperty({ description: '设计文档', type: () => [DocumentReferenceSwagger] })
  designDocuments: DocumentReferenceSwagger[]

  @ApiProperty({ description: 'PRD文档', type: () => [DocumentReferenceSwagger] })
  prdDocuments: DocumentReferenceSwagger[]

  @ApiProperty({ description: 'OpenAPI文档', type: () => [DocumentReferenceSwagger] })
...
export class ProjectSwagger {
  ...
  @ApiProperty({ description: '项目状态', enum: ['active', 'paused', 'completed', 'archived'] })
  status: string
