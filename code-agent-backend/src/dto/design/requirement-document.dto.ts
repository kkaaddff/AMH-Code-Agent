...
export class GenerateRequirementDocumentBody {
  ...
  @ApiProperty({
    required: false,
    example: true,
    description: '是否强制从 DSL 解析最新数据',
  })
  forceRefresh?: boolean;

  @ApiProperty({
    required: false,
    description: '组件标注根节点数据',
    type: 'object',
  })
...
export class RequirementDocumentListResponse extends AsyncResponse {
  @ApiProperty({
    type: [DesignRequirementDocumentEntity],
    description: '需求文档列表',
  })
...
export class RequirementDocumentExportResponse extends AsyncResponse {
  @ApiProperty({
    example: 'https://oss/download/path',
