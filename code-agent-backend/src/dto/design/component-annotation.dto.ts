...
export class SaveDesignAnnotationBody {
  @ApiProperty({ required: false, example: 2, description: '标注版本号，自增。缺省时由服务端生成' })
  version?: number

  @ApiProperty({ required: true, description: '标注根节点数据结构', type: 'object' })
  rootAnnotation: Record<string, unknown>

  @ApiProperty({ required: false, description: '展开的节点 ID 列表', type: [String] })
  expandedKeys?: string[]

  @ApiProperty({ required: false, example: '1.0.0', description: '标注协议版本' })
  schemaVersion?: string

  @ApiProperty({ required: false, example: true, description: '是否覆盖现有版本' })
  force?: boolean
}
...
export class GetDesignAnnotationResponse extends AsyncResponse {
  @ApiProperty({ type: DesignComponentAnnotationEntity, description: '标注实体' })
...
export class DiffChangeSetItem {
  ...
  @ApiProperty({ example: 'added', enum: ['added', 'removed', 'updated'], description: '变更类型' })
