...
@index({ designId: 1, status: 1 }, { name: 'design_requirement_docs_design_status_idx' })
...
export class DesignRequirementDocumentEntity {
  ...
  @ApiProperty({ example: 'draft', enum: ['draft', 'published', 'archived'], description: '文档状态' })
  @prop({ default: 'draft' })
  public status: RequirementDocumentStatus

  @ApiProperty({ example: 'oss://bucket/path/to/doc.md', description: '导出文件 OSS Key' })
  @prop()
  public ossObjectKey?: string

  @ApiProperty({ example: ['md', 'pdf'], description: '可用导出格式', type: [String] })
