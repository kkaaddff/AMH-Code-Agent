...
export type CodeGenerationTaskStatus = 'pending' | 'processing' | 'completed' | 'failed' | 'canceled'

@EntityModel()
@modelOptions({ schemaOptions: { timestamps: true }, options: { allowMixed: Severity.ALLOW } })
export class CodeGenerationTaskResult {
  @ApiProperty({ example: 'oss://bucket/zip/task-001.zip', description: '生成结果 ZIP 的 OSS Key' })
  @prop()
...
@index({ designId: 1, status: 1 }, { name: 'design_code_tasks_design_status_idx' })
