...
import {
  DesignCodeGenerationTaskService,
  CodeGenerationQueuePayload,
} from "../../service/design";
...
export class DesignCodeGenerationProcessor {
  ...
  public async execute(
    data: CodeGenerationQueuePayload,
    job: Job
  ): Promise<void> {
    const { taskId } = data;
    try {
      await this.designCodeGenerationTaskService.markProcessing(
        taskId,
        `Job ${job.id} started`
      );
      const context =
        await this.designCodeGenerationTaskService.loadTaskContext(taskId);

      await this.designCodeGenerationTaskService.updateProgress(
        taskId,
        25,
        "解析设计 DSL 与标注"
      );
      ...
      if (context.requirementDoc?.content) {
        zip.addFile(
          "requirement.md",
          Buffer.from(context.requirementDoc.content, "utf8")
        );
      }
      ...
      await this.designCodeGenerationTaskService.failTask(
        taskId,
        error as Error
      );
      throw error;
