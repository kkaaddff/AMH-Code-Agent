...
export class DesignDocumentController {
  ...
  async createDesign(@Body() body: CreateDesignDocumentBody) {
    ...
    const result = await this.designDocumentService.createDesignDocument(body, operatorId)
  ...
  async paginateDesigns(@Query() query: DesignDocumentPaginationQuery) {
    const { list, total } = await this.designDocumentService.paginateDesignDocuments(query)
  ...
  async getDesignDetail(@Param('designId') designId: string) {
    const doc = await this.designDocumentService.getDesignDocumentById(designId)
    if (!doc) {
  ...
  @ApiOkResponse({ type: DesignDocumentDetailResponse })
  @Put('/:designId')
  async updateDesign(@Param('designId') designId: string, @Body() body: UpdateDesignDocumentBody) {
    const operatorId = this.resolveOperatorId()
    const result = await this.designDocumentService.updateDesignDocument(designId, body, operatorId)
    return new DesignDocumentDetailResponse(result)
  }
  ...
  @Get('/:designId/dsl')
  async getDesignDsl(@Param('designId') designId: string, @Query('revision') revision?: string) {
    const parsedRevision = revision !== undefined ? Number(revision) : undefined
    const revisionNum = Number.isFinite(parsedRevision) ? (parsedRevision as number) : undefined
    const { dsl, revision: currentRevision } = await this.designDocumentService.getDesignDsl(designId, revisionNum)
