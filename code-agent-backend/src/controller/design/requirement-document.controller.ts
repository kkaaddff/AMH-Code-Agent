...
export class DesignRequirementDocumentController {
  ...
  async generateRequirementDocument(
    @Body() body: GenerateRequirementDocumentBody & { designId: string }
  ) {
    ...
    try {
      await this.designRequirementDocumentService.streamRequirementDocument(
        designId,
        restBody,
        operatorId,
        {
  ...
  async updateRequirementDocument(
    @Body() body: UpdateRequirementDocumentBody & { docId: string }
  ) {
    const { docId, ...restBody } = body;
    const operatorId = this.resolveOperatorId();
    await this.designRequirementDocumentService.updateRequirementDocument(
      docId,
      restBody,
      operatorId
    );
  ...
  async getRequirementDocument(@Query() query: { docId: string }) {
    const { docId } = query;
    const doc =
      await this.designRequirementDocumentService.getRequirementDocumentById(
        docId
      );
    if (!doc) {
  ...
  async listRequirementDocuments(
    @Query() query: RequirementDocumentPaginationQuery & { designId: string }
  ) {
    const { designId, ...restQuery } = query;
    const { list, total } =
      await this.designRequirementDocumentService.paginateRequirementDocuments(
        designId,
        restQuery
      );
  ...
  async exportRequirementDocument(@Body() body: { docId: string }) {
    ...
    const operatorId = this.resolveOperatorId();
    const downloadUrl =
      await this.designRequirementDocumentService.exportRequirementDocument(
        docId,
        operatorId
      );
