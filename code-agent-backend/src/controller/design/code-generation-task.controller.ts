...
export class CodeGenerationTaskController {
  ...
  async createTask(@Param('designId') designId: string, @Body() body: CreateCodeGenerationTaskBody) {
    const operatorId = this.resolveOperatorId()
    const task = await this.designCodeGenerationTaskService.createTask(designId, body, operatorId)
  ...
  async listTasks(@Param('designId') designId: string, @Query() query: CodeGenerationTaskPaginationQuery) {
    const { list, total } = await this.designCodeGenerationTaskService.paginateTasks(designId, query)
  ...
  async downloadTaskResult(@Param('taskId') taskId: string) {
    ...
    if (task.status !== 'completed' || !task.result?.outputZipKey) {
      throw new MidwayHttpError('Task result not available yet', HttpStatus.BAD_REQUEST)
    }
