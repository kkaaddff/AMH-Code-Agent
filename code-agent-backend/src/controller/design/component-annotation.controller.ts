...
export class DesignComponentAnnotationController {
  ...
  async saveAnnotation(
    ...
    @Body() body: SaveDesignAnnotationBody
  ) {
    const operatorId = this.resolveOperatorId();
    const annotation =
      await this.designComponentAnnotationService.saveAnnotation(
        designId,
        body,
        operatorId
      );
  ...
  async getAnnotation(
    @Param("designId") designId: string,
    @Query() query: GetDesignAnnotationQuery
  ) {
    const version =
  ...
  async diffAnnotations(
    @Param("designId") designId: string,
    @Query() query: DiffDesignAnnotationQuery
  ) {
    ...
    const diff = await this.designComponentAnnotationService.diffAnnotations(
      designId,
      {
        fromVersion,
        toVersion,
