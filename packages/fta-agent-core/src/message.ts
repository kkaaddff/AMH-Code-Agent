...
export function toolResultPart2ToToolResultPart(part: ToolResultPart2): ToolResultPart {
  return {
    type: 'tool_result',
    id: part.toolCallId,
    name: part.toolName,
...
export function getMessageText(message: Message) {
  if ('uiContent' in message && message.uiContent && typeof message.uiContent === 'string') {
...
export function findIncompleteToolUses(messages: NormalizedMessage[]): {
  ...
  const toolUses = lastAssistantMessage.content.filter((part): part is ToolUsePart => part.type === 'tool_use');
  ...
  const incompleteToolUses = toolUses.filter((toolUse) => !completedToolIds.has(toolUse.id));
