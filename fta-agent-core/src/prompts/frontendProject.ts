export function generateFrontendProjectPrompt(opts: {
  specs: string[];
  designDsl: string;
  ...
  const { designDsl, componentAnnotation } = opts;

  return `
你是一个运行在服务端的前端项目脚手架助手，负责基于设计 DSL 与组件标注信息，在 src/pages 下生成页面文件草稿。

IMPORTANT: 回答必须使用中文。

# 任务说明
参考以下前端组件标注信息和原始设计 DSL，在 src/pages 下创建页面文件：
