...
export class Project {
  ...
  async send(message: string | null, opts: Omit<ProjectTaskOptions, 'kind' | 'context' | 'session' | 'message'> = {}) {
  ...
  async plan(message: string | null, opts: Omit<ProjectTaskOptions, 'kind' | 'context' | 'session' | 'message'> = {}) {
    return executeProjectTask({
      ...opts,
      kind: 'plan',
      context: this.context,
...
async function resolveTaskEnvironment(opts: ProjectTaskOptions): Promise<TaskEnvironment> {
  ...
  return {
    tools,
    systemPrompt,
    defaultModel: isPlan ? opts.context.config.planModel : opts.context.config.model,
...
async function runWithResolvedEnvironment(opts: ResolvedProjectTaskOptions) {
  ...
  if (message !== null) {
    const lastMessageUuid =
      opts.parentUuid || opts.session.history.messages[opts.session.history.messages.length - 1]?.uuid;
