...
export function createBashTool(opts: { cwd: string; backgroundTaskManager: BackgroundTaskManager }) {
...
`.trim(),
    ...
    execute: async ({ command, timeout = DEFAULT_TIMEOUT, run_in_background }) => {
      try {
        ...
        return await executeCommand(command, timeout || DEFAULT_TIMEOUT, cwd, run_in_background, backgroundTaskManager);
      } catch (e) {
        return {
          isError: true,
          llmContent:
