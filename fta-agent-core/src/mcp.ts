...
export class MCPManager {
  ...
  private async _connectServer(key: string, config: MCPConfig): Promise<void> {
    ...
    try {
      ...
      debug(`MCP server connected successfully: ${key}, tools: ${Object.keys(tools).length}`);
    } catch (error) {
  ...
  async getAllServerStatus(): Promise<Record<string, { status: MCPServerStatus; error?: string; toolCount: number }>> {
  ...
  private async _testConnectionAndFetchTools(config: MCPConfig): Promise<{ client: any; tools: Record<string, any> }> {
  ...
  #convertAiSdkToolToLocal(toolName: string, toolDef: any, serverName: string, config: MCPConfig): Tool {
    return {
...
export function convertMcpResultToLlmContent(result: any): string | (TextPart | ImagePart)[] {
  ...
  const isImagePart = (part: object) => {
    return 'type' in part && part.type === 'image' && 'data' in part && 'mimeType' in part;
  };
